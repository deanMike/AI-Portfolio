;;Scouts have the same proposal functions as users but with the ability to drop flags near good things.

(ns pucks.agents.scout
  (:use [pucks globals util vec2D]
        [pucks.agents active]))


 #_(defn scout-proposals [p]
   {:acceleration 1
    :rotation (direction->rotation (:velocity p))})

 (defn scout-proposals [p]
   ...
   {:acceleration (if (and (< (:energy p) 0.6) (not (empty? (filter :vent (:overlaps p)))))
    ...
    :spawn  (if (empty? (filter :flag (:overlaps p)))
              (if (or (not (empty? (filter :vent (:overlaps p)))) 
                      (not (empty? (filter :gate (:overlaps p)))) 
                      (not (empty? (filter :chest (:overlaps p)))))
...
(defn scout [spawn-function]
  (merge (active)
         {:scout true
          :color [100 100 100]
         :proposal-function scout-proposals
